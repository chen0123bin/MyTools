...
namespace LWNodeEditor {
    ...
    [CustomNodeGraphEditor(typeof(LWNode.LWNodeGraph))]
    public class NodeGraphEditor : LWNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, LWNode.LWNodeGraph> {
        ...
        public virtual string GetNodeMenuName(Type type) {
            ...
            LWNode.Node.CreateNodeMenuAttribute attrib;
        ...
        public virtual void AddContextMenuItems(GenericMenu menu) {
            ...
            for (int i = 0; i < nodeTypes.Length; i++) {
                ...
                if (disallowed) menu.AddItem(new GUIContent(path), false, null);
                    ...
                    LWNode.Node node = CreateNode(type, pos);
        ...
        public virtual Gradient GetNoodleGradient(LWNode.NodePort output, LWNode.NodePort input) {
        ...
        /// <param name="output"> The output this noodle comes from. Never null. </param>
        /// <param name="input"> The output this noodle comes from. Can be null if we are dragging the noodle. </param>
        public virtual float GetNoodleThickness(LWNode.NodePort output, LWNode.NodePort input) {
            return 5f;
        }

        public virtual NoodlePath GetNoodlePath(LWNode.NodePort output, LWNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodlePath;
        }

        public virtual NoodleStroke GetNoodleStroke(LWNode.NodePort output, LWNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodleStroke;
        }

        /// <summary> Returned color is used to color ports </summary>
        public virtual Color GetPortColor(LWNode.NodePort port) {
        ...
        public virtual string GetPortTooltip(LWNode.NodePort port) {
        ...
        public virtual LWNode.Node CreateNode(Type type, Vector2 position) {
            Undo.RecordObject(target, "Create Node");
            LWNode.Node node = target.AddNode(type);
        ...
        public virtual LWNode.Node CopyNode(LWNode.Node original) {
            Undo.RecordObject(target, "Duplicate Node");
            LWNode.Node node = target.CopyNode(original);
        ...
        public virtual bool CanRemove(LWNode.Node node) {
            // Check graph attributes to see if this node is required
            Type graphType = target.GetType();
            LWNode.LWNodeGraph.RequireNodeAttribute[] attribs = Array.ConvertAll(
                graphType.GetCustomAttributes(typeof(LWNode.LWNodeGraph.RequireNodeAttribute), true), x => x as LWNode.LWNodeGraph.RequireNodeAttribute);
        ...
        public virtual void RemoveNode(LWNode.Node node) {
        ...
        LWNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, LWNode.LWNodeGraph>.INodeEditorAttrib {
