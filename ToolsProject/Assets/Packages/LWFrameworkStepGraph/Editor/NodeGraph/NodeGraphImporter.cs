    ...
    class NodeGraphImporter : AssetPostprocessor {
        private static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths) {
            foreach (string path in importedAssets) {
                ...
                LWNodeGraph graph = AssetDatabase.LoadAssetAtPath<LWNodeGraph>(path);
                if (graph == null) continue;

                // Get attributes
                Type graphType = graph.GetType();
                LWNodeGraph.RequireNodeAttribute[] attribs = Array.ConvertAll(
                    graphType.GetCustomAttributes(typeof(LWNodeGraph.RequireNodeAttribute), true), x => x as LWNodeGraph.RequireNodeAttribute);

                Vector2 position = Vector2.zero;
                foreach (LWNodeGraph.RequireNodeAttribute attrib in attribs) {
        ...
        private static void AddRequired(LWNodeGraph graph, Type type, ref Vector2 position) {
            if (!graph.nodes.Any(x => x.GetType() == type)) {
                LWNode.Node node = graph.AddNode(type);
