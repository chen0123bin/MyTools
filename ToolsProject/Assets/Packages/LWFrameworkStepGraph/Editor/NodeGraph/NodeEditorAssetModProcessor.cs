...
namespace XNodeEditor {
    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor {
        ...
        private static AssetDeleteResult OnWillDeleteAsset (string path, RemoveAssetOptions options) {
            ...
            if (scriptType == null || (scriptType != typeof (LWNode.Node) && !scriptType.IsSubclassOf (typeof (LWNode.Node)))) return AssetDeleteResult.DidNotDelete;
            ...
            for (int i = 0; i < guids.Length; i++) {
                ...
                for (int k = 0; k < objs.Length; k++) {
                    LWNode.Node node = objs[k] as LWNode.Node;
        ...
        private static void OnReloadEditor () {
            ...
            string[] guids = AssetDatabase.FindAssets ("t:" + typeof (LWNode.LWNodeGraph));
            for (int i = 0; i < guids.Length; i++) {
                string assetpath = AssetDatabase.GUIDToAssetPath (guids[i]);
                LWNode.LWNodeGraph graph = AssetDatabase.LoadAssetAtPath (assetpath, typeof (LWNode.LWNodeGraph)) as LWNode.LWNodeGraph;
                ...
                for (int u = 0; u < objs.Length; u++) {
                    ...
                    if (!graph.nodes.Contains (objs[u] as LWNode.Node)) graph.nodes.Add(objs[u] as LWNode.Node);
                }
            }
        }
    }
