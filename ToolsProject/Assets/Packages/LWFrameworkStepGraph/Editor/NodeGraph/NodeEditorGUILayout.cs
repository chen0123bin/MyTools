...
namespace LWNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            LWNode.Node node = property.serializedObject.targetObject as LWNode.Node;
            LWNode.NodePort port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, LWNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            PropertyField(property, null, port, includeChildren, options);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, LWNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            if (property == null) throw new NullReferenceException();
            ...
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, GUILayout.MinWidth(30));
                ...
                if (port.direction == LWNode.NodePort.IO.Input) {
                    // Get data from [Input] attribute
                    LWNode.Node.ShowBackingValue showBacking = LWNode.Node.ShowBackingValue.Unconnected;
                    LWNode.Node.InputAttribute inputAttribute;
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                        showBacking == LWNode.Node.ShowBackingValue.Never ||
                        (showBacking == LWNode.Node.ShowBackingValue.Unconnected && port.IsConnected);
                    ...
                    if (dynamicPortList) {
                        ...
                        LWNode.Node.ConnectionType connectionType = inputAttribute != null ? inputAttribute.connectionType : LWNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
                        case LWNode.Node.ShowBackingValue.Unconnected:
                ...
                } else if (port.direction == LWNode.NodePort.IO.Output) {
                    // Get data from [Output] attribute
                    LWNode.Node.ShowBackingValue showBacking = LWNode.Node.ShowBackingValue.Unconnected;
                    LWNode.Node.OutputAttribute outputAttribute;
                    bool dynamicPortList = false;
                    ...
                    if (dynamicPortList) {
                        Type type = GetType(property);
                        LWNode.Node.ConnectionType connectionType = outputAttribute != null ? outputAttribute.connectionType : LWNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
        ...
        public static void PortField(LWNode.NodePort port, params GUILayoutOption[] options) {
            PortField(null, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, LWNode.NodePort port, params GUILayoutOption[] options) {
            if (port == null) return;
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, LWNode.NodePort port) {
            if (port == null) return;
        ...
        public static void AddPortField(LWNode.NodePort port) {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(LWNode.NodePort input, LWNode.NodePort output) {
...
#region Obsolete
        ...
        public static bool IsInstancePortListPort(LWNode.NodePort port) {
            return IsDynamicPortListPort(port);
        }

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type, SerializedObject serializedObject, LWNode.NodePort.IO io, LWNode.Node.ConnectionType connectionType = LWNode.Node.ConnectionType.Multiple, LWNode.Node.TypeConstraint typeConstraint = LWNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
        ...
        public static bool IsDynamicPortListPort(LWNode.NodePort port) {
        ...
        public static void DynamicPortList(string fieldName, Type type, SerializedObject serializedObject, LWNode.NodePort.IO io, LWNode.Node.ConnectionType connectionType = LWNode.Node.ConnectionType.Multiple, LWNode.Node.TypeConstraint typeConstraint = LWNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
            LWNode.Node node = serializedObject.targetObject as LWNode.Node;
            ...
            var indexedPorts = node.DynamicPorts.Select(x => {
                ...
                return new { index = -1, port = (LWNode.NodePort) null };
            }).Where(x => x.port != null);
            List<LWNode.NodePort> dynamicPorts = indexedPorts.OrderBy(x => x.index).Select(x => x.port).ToList();
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<LWNode.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, LWNode.NodePort.IO io, LWNode.Node.ConnectionType connectionType, LWNode.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    LWNode.NodePort port = node.GetPort(fieldName + " " + index);
                ...
                (ReorderableList rl) => {
                    ...
                    if (rl.index > reorderableListIndex) {
                        for (int i = reorderableListIndex; i < rl.index; ++i) {
                            LWNode.NodePort port = node.GetPort(fieldName + " " + i);
                            LWNode.NodePort nextPort = node.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    if (io == LWNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, LWNode.Node.TypeConstraint.None, newName);
                    ...
                    if (dynamicPorts[index] == null) {
                        ...
                        for (int k = index + 1; k < dynamicPorts.Count(); k++) {
                            for (int j = 0; j < dynamicPorts[k].ConnectionCount; j++) {
                                LWNode.NodePort other = dynamicPorts[k].GetConnection(j);
            ...
            if (hasArrayData) {
                ...
                while (dynamicPortCount < arrayData.arraySize) {
                    ...
                    if (io == LWNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else node.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(node);
                    dynamicPortCount++;
                }
