...
namespace LWNodeEditor {
    ...
    /// is the main asset (present at top level) between the <see cref="LWNode.LWNodeGraph"/> and one of its <see cref="LWNode.Node"/>
    /// sub-assets. As a workaround until Unity fixes this, this asset processor checks all renamed assets and if it
    /// finds a case where a <see cref="LWNode.Node"/> has been made the main asset it will swap it back to being a sub-asset
