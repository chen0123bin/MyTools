...
namespace LWNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        private List<LWNode.Node> culledNodes;
        ...
        void ShowPortContextMenu(LWNode.NodePort hoveredPort) {
            GenericMenu contextMenu = new GenericMenu();
            ...
            foreach (LWNode.Node node in graph.nodes) {
                ...
                foreach (LWNode.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        LWNode.NodePort input = output.GetConnection(k);
        ...
        private void DrawNodes() {
            ...
            if (Selection.activeObject != null && Selection.activeObject is LWNode.Node) {
            ...
            List<LWNode.NodePort> removeEntries = new List<LWNode.NodePort>();

            if (e.type == EventType.Layout) culledNodes = new List<LWNode.Node>();
            for (int n = 0; n < graph.nodes.Count; n++) {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[n] == null) continue;
                if (n >= graph.nodes.Count) return;
                LWNode.Node node = graph.nodes[n];
        ...
        private bool ShouldBeCulled(LWNode.Node node) {
