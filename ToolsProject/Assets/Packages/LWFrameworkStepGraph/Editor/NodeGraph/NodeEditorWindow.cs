...
namespace LWNodeEditor {
    ...
    public partial class NodeEditorWindow : EditorWindow {
        ...
        public Dictionary<LWNode.NodePort, Rect> portConnectionPoints { get { return _portConnectionPoints; } }
        private Dictionary<LWNode.NodePort, Rect> _portConnectionPoints = new Dictionary<LWNode.NodePort, Rect>();
        ...
        [System.Serializable] private class NodePortReference {
            [SerializeField] private LWNode.Node _node;
            [SerializeField] private string _name;

            public NodePortReference(LWNode.NodePort nodePort) {
                _node = nodePort.node;
                _name = nodePort.fieldName;
            }

            public LWNode.NodePort GetNodePort() {
                if (_node == null) {
                    return null;
                }
        ...
        private void OnEnable() {
            ...
            if (length == _rects.Length) {
                for (int i = 0; i < length; i++) {
                    LWNode.NodePort nodePort = _references[i].GetNodePort();
                    if (nodePort != null)
        ...
        public Dictionary<LWNode.Node, Vector2> nodeSizes { get { return _nodeSizes; } }
        private Dictionary<LWNode.Node, Vector2> _nodeSizes = new Dictionary<LWNode.Node, Vector2>();
        public LWNode.LWNodeGraph graph;
        ...
        private static void OnSelectionChanged() {
            LWNode.LWNodeGraph nodeGraph = Selection.activeObject as LWNode.LWNodeGraph;
        ...
        public void SaveAs() {
            ...
            if (string.IsNullOrEmpty(path)) return;
                ...
                LWNode.LWNodeGraph existingGraph = AssetDatabase.LoadAssetAtPath<LWNode.LWNodeGraph>(path);
        ...
        public void SelectNode(LWNode.Node node, bool add) {
            if (add) {
        ...
        public void DeselectNode(LWNode.Node node) {
            List<Object> selection = new List<Object>(Selection.objects);
        ...
        public static bool OnOpen(int instanceID, int line) {
            LWNode.LWNodeGraph nodeGraph = EditorUtility.InstanceIDToObject(instanceID) as LWNode.LWNodeGraph;
            if (nodeGraph != null) {
                Open(nodeGraph);
                return true;
            }
