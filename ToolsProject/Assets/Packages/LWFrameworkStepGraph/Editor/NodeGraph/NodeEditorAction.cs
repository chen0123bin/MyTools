...
namespace LWNodeEditor {
    public partial class NodeEditorWindow {
        ...
        public static LWNode.Node[] copyBuffer = null;
        ...
        private LWNode.Node hoveredNode = null;
        [NonSerialized] public LWNode.NodePort hoveredPort = null;
        [NonSerialized] private LWNode.NodePort draggedOutput = null;
        [NonSerialized] private LWNode.NodePort draggedOutputTarget = null;
        [NonSerialized] private LWNode.NodePort autoConnectOutput = null;
            ...
            switch (e.type) {
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is LWNode.Node) {
                                    LWNode.Node node = Selection.objects[i] as LWNode.Node;
                                    ...
                                    if (offset.sqrMagnitude > 0) {
                                        foreach (LWNode.NodePort output in node.Outputs) {
                                            Rect rect;
                                        ...
                                        foreach (LWNode.NodePort input in node.Inputs) {
                                            Rect rect;
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    LWNode.Node node = hoveredPort.node;
                                    LWNode.NodePort output = hoveredPort.Connection;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null && draggedOutput.CanConnectTo(draggedOutputTarget)) {
                                LWNode.Node node = draggedOutputTarget.node;
                            ...
                            IEnumerable<LWNode.Node> nodes = Selection.objects.Where(x => x is LWNode.Node).Select(x => x as LWNode.Node);
                            foreach (LWNode.Node node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
                ...
                case EventType.KeyDown:
                    ...
                    if (e.keyCode == KeyCode.A) {
                        if (Selection.objects.Any(x => graph.nodes.Contains(x as LWNode.Node))) {
                            foreach (LWNode.Node node in graph.nodes) {
                                DeselectNode(node);
                            }
                        } else {
                            foreach (LWNode.Node node in graph.nodes) {
        ...
        public void Home() {
            var nodes = Selection.objects.Where(o => o is LWNode.Node).Cast<LWNode.Node>().ToList();
            if (nodes.Count > 0) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is LWNode.Node) {
                    LWNode.Node node = item as LWNode.Node;
        ...
        public void RenameSelectedNode() {
            if (Selection.objects.Length == 1 && Selection.activeObject is LWNode.Node) {
                LWNode.Node node = Selection.activeObject as LWNode.Node;
                Vector2 size;
        ...
        public void MoveNodeToTop(LWNode.Node node) {
            int index;
        ...
        public void DuplicateSelectedNodes() {
            ...
            LWNode.Node[] selectedNodes = Selection.objects.Select(x => x as LWNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        ...
        public void CopySelectedNodes() {
            copyBuffer = Selection.objects.Select(x => x as LWNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(LWNode.Node[] nodes, Vector2 topLeft) {
            ...
            Dictionary<LWNode.Node, LWNode.Node> substitutes = new Dictionary<LWNode.Node, LWNode.Node>();
            for (int i = 0; i < nodes.Length; i++) {
                LWNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;

                // Check if user is allowed to add more of given node type
                LWNode.Node.DisallowMultipleNodesAttribute disallowAttrib;
                ...
                LWNode.Node newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < nodes.Length; i++) {
                LWNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;
                foreach (LWNode.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        LWNode.NodePort inputPort = port.direction == LWNode.NodePort.IO.Input ? port : port.GetConnection(c);
                        LWNode.NodePort outputPort = port.direction == LWNode.NodePort.IO.Output ? port : port.GetConnection(c);

                        LWNode.Node newNodeIn, newNodeOut;
        ...
        bool IsHoveringTitle(LWNode.Node node) {
        ...
        public void AutoConnect(LWNode.Node node) {
            if (autoConnectOutput == null) return;

            // Find input port of same type
            LWNode.NodePort inputPort = node.Ports.FirstOrDefault(x => x.IsInput && x.ValueType == autoConnectOutput.ValueType);
